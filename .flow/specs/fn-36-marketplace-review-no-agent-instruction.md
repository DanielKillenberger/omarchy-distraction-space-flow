...
- **R1a:** `.flow/` is not tracked either and is listed in `.gitignore`; the flow state and the instruction files live in the sibling repository `omarchy-distraction-space-flow`, reached from each checkout through a `.flow` symlink, per the reviewer's "keep development orchestration material outside the repository that Omarchy installs" and the user's decision. [paraphrase]
...
- The sibling repository is not published with the plugin and is never referenced by the installed tree. [inferred]
